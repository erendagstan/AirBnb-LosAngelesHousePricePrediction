...
df = pd.read_csv("DatalantaProject/datasets/la_dataframe_v3.csv", low_memory=False)
...
plt.title('Top 20 Amenities with Normalized Frequencies')
...
df.columns.tolist
df = df.drop(['unnamed:_0.1.1'], axis=1)
...
df = pd.read_csv("/Users/Furkan/Desktop/DATALANTA.csv")
...
X.isnull().sum()
X.drop(["_host_activity_score","_host_activity_score","host_duration"],axis=1,inplace=True)
X.drop(["host_duration"],axis=1,inplace=True)
import joblib
joblib.dump(best_gb_model_random_search, "DatalantaProject/model/gb_model.joblib")
...
print("TRAIN RMSE")
y_pred = best_xgb_model_random_search.predict(X_train)
print(np.sqrt(mean_squared_error(y_train, y_pred)))
# 0.15819943435028702
print("TRAIN R KARE")
print(best_xgb_model_random_search.score(X_train, y_train))
# 0.9407409449865263

print("TEST RMSE")
y_pred = best_xgb_model_random_search.predict(X_test)
print(np.sqrt(mean_squared_error(y_test, y_pred)))
# 0.2844557852041956

print("TEST R KARE")
print(best_xgb_model_random_search.score(X_test, y_test))
# 0.8148474114165418

# Eren
import joblib

joblib.dump(best_xgb_model_random_search, "DatalantaProject/model/xgb_model.joblib")
X.to_csv("DatalantaProject/datasets/model_datalanta_dataset.csv")

ber = X.sample().values
prediction = best_xgb_model_random_search.predict(ber)[0]
np.exp(prediction)
...
joblib.dump(best_lgbm_model_random_search, "DatalantaProject/model/lgbm_model.joblib")
##############
